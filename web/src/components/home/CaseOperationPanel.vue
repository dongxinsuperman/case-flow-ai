...
function lanesForCase(item: CaseListItem): Array<{ key: CoverageLane; label: string }> {
  return coverageLanesByTarget[item.executionTarget] ?? []
}

function coverageState(item: CaseListItem, lane: CoverageLane): CoverageState {
  return item.coverage?.[lane] ?? 'none'
}

function bugCount(item: CaseListItem): number {
  return item.bugs?.length ?? 0
}

function coverageClass(item: CaseListItem, lane: CoverageLane) {
...
function elapsedText(item: CaseListItem) {
...
function executeButtonText(item: CaseListItem) {
...
function failureTypeMeta(item: CaseListItem) {
          ...
          <div class="case-controls" @click.stop>
            ...
            <button
              ...
              :class="{
                ...
                preparing: bugCount(item) === 0 && item.diagnosisReady && !item.bugDraftReady && !!item.reportUrl,
              }"
              type="button"
              data-control="submit-bug"
              :title="bugCount(item) > 0 ? '点击查看已提交 bug / 再提一条' : (item.bugDraftReady ? '' : '后台预填准备中…')"
