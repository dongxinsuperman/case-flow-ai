...
- `function_map`：逐份读取本次挂载的 Function Map，作为按端编排和调用子执行器的参考。
...
当前资产挂载流程编译提交级 `functionMapContext`，Hybrid 在调用 AI Phone、AI Web 或 AI API 子工具时原样透传。标准模式和快速模式还会把同一批已筛选资产作为结构化 `functionMaps` 一并交给 Hybrid；模型首轮只看到其中的目录（资产 ID、标题、简介和适用端），再用 `function_map` 按 ID 逐份读取全部正文。

每份 Map 都是参考信息：Hybrid 按 `targets` 只把 App、Web、API Map 用于对应端的编排和子任务输入，不能跨端套用，也不能把 Map 当成固定流程或最终结论。结构化目录中端未知的 Map 不得被擅自归类。

账号与设备映射仍是其中一种参考。例如，Map 明确「教师账号使用 `teacher-device`」且当前步骤需要该账号时，Hybrid 会在调用 AI Phone 时指定该 alias。此时系统只会校验和等待这台设备：设备离线、别名多义、平台冲突或等待超时都会返回 `needs_human`，**绝不会换另一台设备继续执行**。没有明确绑定时，才保留 AI Phone 原有的按平台资源池调度。
