...
- 设备：ai_phone 未明确端 / 系统 → 默认 android，不要填 platform；没有明确设备绑定时不要填 device_alias。
...
8. 设备硬锁：只有 Map 或 case 能唯一确定「本步骤账号/角色 → 某 alias」时才填 ai_phone.device_alias。填了即只锁该设备：离线、别名多义、平台冲突或占用超时都必须停止并 `needs_human`，绝不换别的设备。没有绑定才不填，沿用正常按端选空闲池；快照失败不能把已有绑定改回默认池。无法唯一判断时 finish=needs_human，不猜测设备。
...
TOOL_SPECS = [
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "device_alias": {
                    "type": "string",
                    "description": "可选的设备硬锁。仅在 case/Function Map 明确账号或角色绑定到某台设备时填写真实 alias；占用时只等待该设备，无法取得时停止，不会替换设备。",
                },
            },
            "required": ["title", "preconditions", "steps", "expected"],
        },
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "asset_id": {"type": "string", "description": "function_map_catalog 中的资产 ID 或标题；留空返回目录。"},
            },
            "required": [],
        },
    },
